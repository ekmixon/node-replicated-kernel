...
def qemu_proc(pid):
	...
	if os.getuid() != 0:
		raise argparse.ArgumentTypeError(f'must be root to manipulate PID {pid}')
	...
	if not procs:
		raise argparse.ArgumentTypeError(f'pid {pid} does not exist.')

	p = procs[0]

	exe = (p / 'exe').resolve()
	if not QEMU_COMM_RE.match(exe.name):
		raise argparse.ArgumentTypeError(f'pid {pid} is not a qemu-system process.')

	cmdline = (p / 'cmdline').read_text().split('\0')

	if not list(filter(QEMU_DEBUG_RE.search, cmdline)):
...
def thread_selector(tsel):
	...
	return ALL_THREADS if tsel == '*' else tsel
...
class ThreadAffinityAction(argparse.Action):
	def __call__(self, parser, namespace, values, option_string=None):
		...
		if list(filter(lambda c: c > 1, selectors.values())):
			parser.error(
				f'argument {argparse._get_action_name(self)}: duplicate affinity value.'
			)
...
def main():
	...
	def cores_affinityspec(cores):
		...
		spec_groups = ','.join(
			[f'{g[0]}-{g[-1]}' if len(g) > 1 else str(g[0]) for g in spec_groups]
		)
